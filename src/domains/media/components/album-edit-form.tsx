...
export const AlbumEditForm: React.FC<{
  ...
  const formData = formStateById[album.id] ?? {
    name: album.name,
  ...
  const sortOptions = useMemo(
    () => [
      {
        id: "recent",
        label: t("form.sort.recent", { default: "Recently added" } as any),
      },
      {
        id: "name-asc",
        label: t("form.sort.alphabet", { default: "Name (A–Z)" } as any),
      },
      {
        id: "name-desc",
        label: t("form.sort.date", { default: "Date (Z–A)" } as any),
      },
    ],
    [t]
  );
  const [sortBy, setSortBy] = useState<{ id: string; label: string } | null>(
    null
  );
  useEffect(() => {
    setSortBy(sortOptions[0] ?? null);
  }, [sortOptions]);
  ...
  useEffect(() => {
    ...
    albumMediaQuery.isLoading,
    albumMediaQuery.data,
    existingAlbumMediaIds.join("|"),
  ]);
  ...
  const validate = (): boolean => {
    ...
    if (!formData.name.en.trim()) {
      errs.name = {
        ...(errs.name || {}),
        en: t("form.validation.nameRequiredEn"),
      };
    ...
    if (!formData.name.ne.trim()) {
      errs.name = {
        ...(errs.name || {}),
        ne: t("form.validation.nameRequiredNe"),
      };
  ...
  useEffect(() => {
    ...
    return () => {
      if (container)
        container.removeEventListener("album:submit", handler as EventListener);
      if (typeof document !== "undefined")
        document.removeEventListener("album:submit", handler as EventListener);
  ...
  const submit = async () => {
    console.log(
      "Submitting album edit form for album ID:",
      album.id,
      "with data:",
      formData,
      "and selected media IDs:",
      selectedList
    );
    setSubmitting(true);
    if (!validate()) {
        ...
        <Column lg={16} md={8} sm={4}>
          <FormGroup legendText="">
            <TranslatableField
              ...
              value={formData.name}
              onChange={(val: any) => updateFormField(album.id, "name", val)}
              invalid={!!errors.name}
              invalidText={
                errors.name && (errors.name.en || errors.name.ne)
                  ? `${errors.name.en || ""} ${errors.name.ne || ""}`.trim()
                  : undefined
              }
              activeTab={nameTab}
            ...
            <div className="m--mt-1">
              <TranslatableField
                ...
                onChange={(val: any) =>
                  updateFormField(album.id, "description", val)
                }
                activeTab={descTab}
            ...
            <div className="m--mt-1">
              <FormLabel>
                {t("form.media", { default: "Media" } as any)}
              </FormLabel>
              ...
              <div className="m--mt-1" style={{ width: "100%" }}>
                <Search
                  ...
                  placeholder={t("filters.searchAlbumsPlaceholder", {
                    default: "Search albums...",
                  } as any)}
                  value={mediaSearch}
              ...
              >
                <Dropdown
                  ...
                  onChange={({ selectedItem }) =>
                    handleSortChange(selectedItem as any)
                  }
                />
                <div style={{ position: "relative" }} ref={filtersRef}>
                  <Button
                    ...
                    size="md"
                    renderIcon={Filter}
                    iconDescription={t("filters.open", {
                  ...
                  {filtersOpen && (
                    <div
                      ...
                      style={{
                        ...
                        minWidth: 160,
                        border:
                          "1px solid var(--cds-border-subtle, rgba(0,0,0,0.12))",
                      }}
                    >
                      <button
                        type="button"
                      ...
                      <Tile
                        ...
                        style={{
                          ...
                          border:
                            "1px solid var(--cds-border-subtle, rgba(0,0,0,0.12))",
                          background:
                            "linear-gradient(90deg, rgba(230,230,230,1) 25%, rgba(245,245,245,1) 37%, rgba(230,230,230,1) 63%)",
                      ...
                      >
                        ...
                        <p
                          style={{
                            margin: 0,
                            marginTop: 0,
                            color: "var(--cds-text-02, #6f6f6f)",
                          }}
                        >
                          {tMedia("list.emptyDescription", {
                            default: "No media matched your search",
                          } as any)}
                        </p>
                      </div>
                    </div>
                            ...
                            >
                              ...
                              <div
                                ...
                                title={
                                  selected
                                    ? safeT(
                                        () => t("form.selected" as any),
                                        "Selected"
                                      )
                                    : safeT(
                                        () => t("form.select" as any),
                                        "Select"
                                      )
                                }
                                style={{
                              ...
                              >
                                ...
                                {selected ? (
                                  <CheckmarkFilled size={25} color="blue" />
                                ) : (
                    ...
                    : // keep placeholder tiles if there are no media items
                      Array.from({ length: 6 }).map((_, idx) => (
                        ...
                        >
                          ...
                          >
                            <span className="cds--label">
                              {t("form.preview", { default: "Preview" } as any)}
                            </span>
                          </Tile>
                        </div>
                      ))}
            ...
            <div className="m--mt-1">
              <Toggle
                ...
                onToggle={(checked) =>
                  updateFormField(album.id, "isActive", checked)
                }
              />
            </div>
          </FormGroup>
