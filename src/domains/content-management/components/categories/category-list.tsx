...
export const CategoryList: React.FC<CategoryListProps> = React.memo(({ onCreate }) => {
  ...
  const { data: categoryResponse, isLoading, error } = useCategories({ page, limit: pageSize });
  ...
  const displayCategories = useMemo(() => {
    ...
    if (Array.isArray(categories) && categories.length > limit && total === categories.length) {
  ...
  console.debug('CategoryList pagination state', { page, pageSize, pagination, categoriesLength: categories.length, displayLength: displayCategories.length });
  ...
  const handlePageChange = useCallback(({ page: newPage, pageSize: newPageSize }: { page: number; pageSize: number }) => {
  ...
  const paginationElement = (
    <div className="pagination-wrapper">
      <Pagination
        ...
        pageNumberText={t("pagination.pageNumber", { default: "Page Number" })}
      />
    </div>
  );
              ...
              {displayCategories.map((cat) => (
                <TableRow key={cat.id}>
                  ...
                  <TableCell className="table-cell-right">
                    <RowActions
                      ariaLabel={t("categories.card.actions", { default: "Actions" })}
                      actions={[
                        { key: 'edit', itemText: t("categories.card.edit", { default: "Edit" }), onClick: () => handleEditCategory(cat) },
                        { key: 'delete', itemText: t("categories.card.delete", { default: "Delete" }), onClick: () => handleDeleteCategory(cat), hasDivider: true, isDelete: true }
                      ]}
                    />
                  </TableCell>
                </TableRow>
