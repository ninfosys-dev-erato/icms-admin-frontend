...
export const SliderImagePreview: React.FC<SliderImagePreviewProps> = ({
  mediaId,
  directUrl,
  alt = "Slider image",
  className = "",
  ...
  const renderPlaceholder = () => (
    <div className={`slider-image-preview empty ${className}`}>
      <ImageIcon size={24} />
    </div>
  );
  ...
  if (isLoading) {
    return (
      <div className={`slider-image-preview slider-image-preview-loading ${className}`}>
        <p className="slider-image-preview-loading-text">
          {t("image.preview.loading")}
        </p>
      </div>
    );
  }
  ...
  if (!showImage) {
    return (
      <div className={`slider-image-preview slider-image-preview-hidden ${className}`}>
        {/* hidden */}
      </div>
    );
  }
  ...
  return (
    <div className={`slider-image-preview ${className}`}>
      {imageUrl && (
        <img
          src={imageUrl}
          alt={alt}
